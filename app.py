...
@app.route('/')
def index():
    # Get the current error mode state
...
def signup():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            logger.error(
                f'user="{username}" action="signup" status="failed" error="username exists"')
            ...
            db.session.rollback()
            logger.exception(
                f'user="{username}" action="signup" status="error"')
...
def log_response(response):
    logger.info(f'path="{request.path}" method="{
                request.method}" status="{response.status_code}"')
    return response
...
def login():
    ...
    if request.method == 'POST':
        ...
        if error_mode and randint(0, 1):
            logger.error(f'user="{
                         username}" action="login" status="error" error="random failure due to error mode"')
        ...
        logger.warning(
            f'user="{username}" action="login" status="failed" error="invalid credentials"')
...
def logout():
    ...
    if error_mode and randint(0, 1):
        logger.error(
            f'user="{username}" action="logout" status="error" error="Failed to loggout."')
...
def toggle_error_mode():
    ...
    logger.info(f"Error mode toggled to {
                'on' if session['error_mode'] else 'off'}.")
...
def handle_add_plant(json):
    ...
    if not user_id or (session.get('error_mode', False) and randint(0, 1)):
        ...
        emit('error', {
             'error': 'Failed to add plant due to server error'}, room=request.sid)
        return
    plant_name = json.get('plant_name')
    plant_type = json.get('plant_type')
    new_plant = Plant(name=plant_name, plant_type=plant_type,
                      health_data="Healthy", user_id=user_id)
    db.session.add(new_plant)
    db.session.commit()
    emit('new_plant', {'plant_id': new_plant.id, 'plant_name': new_plant.name,
         'plant_type': new_plant.plant_type}, room=str(user_id))
...
def handle_connect():
    ...
    if user_id:
        ...
        logger.info(f"User {user_id} connected and joined their room with error mode {
                    active_users[user_id]['error_mode']}.")
            ...
            for user_id, user_info in list(active_users.items()):
                try:
                    if user_info['error_mode'] and randint(0, 1):
                        logger.error(f"Failed to send data to: {
                                     user_id}: Will retry later")
                        continue
                    plants = Plant.query.filter_by(user_id=user_id).all()
                    for plant in plants:
                        fake_data = {
                        ...
                        socketio.emit('update_plant', {
                                      'plant_id': plant.id, 'data': fake_data}, room=str(user_id))
                        logger.debug(f"Simulated data for plant {
                                     plant.id} sent to user {user_id}")
                except Exception as e:
                    logger.error(f"Error in simulation thread for user {
                                 user_id}: {str(e)}")
